...
function getProgressionRequirements(vehicleId, totalVehicles = vehicles.length) {
  const id = Math.max(1, Math.min(Number(vehicleId) || 1, totalVehicles));

  // Requested progression anchors:
  // Vehicle 1 = 0.5h / 20 messages
  // Vehicle 2 = 1h / 50 messages
  // Vehicle 3 = 2h / 150 messages
  // Vehicle 4 = 4h / 400 messages
  // Vehicle 115 = 2000h / 200,000 messages
  if (id === 1) return { hours: 0.5, messages: 20 };
  if (id === 2) return { hours: 1, messages: 50 };
  if (id === 3) return { hours: 2, messages: 150 };
  if (id === 4) return { hours: 4, messages: 400 };
  if (id >= totalVehicles) return { hours: 2000, messages: 200000 };

  // Stronger, smooth growth for the remaining vehicles. The square-root curve
  // makes each later vehicle meaningfully harder while still reaching the exact
  // maximum at vehicle 115 instead of producing tiny 0.01h/1-message jumps.
  const t = Math.sqrt((id - 4) / (totalVehicles - 4));
  const multiplier = Math.pow(500, t);
  return {
    hours: Math.round(4 * multiplier * 100) / 100,
    messages: Math.round(400 * multiplier)
  };
}
...
async function checkUnlocks(guild, userId) {
  ...
  if (hours < requirements.hours || messages < requirements.messages) return;

  // Unlock exactly one vehicle per progression check.
  ...
  try {
    ...
    console.log(`Progression unlock checked: guild=${guild.id} user=${userId} vehicle=${unlockIndex} granted=${result?.granted !== false} requirements=${requirements.hours}h/${requirements.messages}msg`);
  } catch (err) {
  ...
  if (channel) {
    await channel.send(
      ...
      `${member} has unlocked **${next.emoji} ${next.name}**!\n` +
