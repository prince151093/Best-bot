...
async function handleActivityCommand(message) {
  ...
  if (lower === "?lock") {
    ...
    if (!message.channel.permissionOverwrites) {
      return message.reply("❌ This channel cannot be locked.");
    }

    try {
      // Explicitly deny sending for @everyone. Using an explicit overwrite
      // makes the lock reliable even when the channel has other inherited
      // permissions.
      ...
      // Re-fetch the overwrite so we know Discord actually saved the lock.
      const everyoneOverwrite =
        message.channel.permissionOverwrites.cache.get(message.guild.roles.everyone.id);

      if (!everyoneOverwrite || everyoneOverwrite.deny.has(PermissionFlagsBits.SendMessages) === false) {
        throw new Error("Discord did not apply the @everyone SendMessages deny.");
      }
      ...
      return message.reply(
        "❌ I couldn't lock this channel. Make sure the bot has **Manage Channels** permission and its role is high enough to manage this channel."
      );
    }
  }
  ...
  if (lower === "?unlock") {
    ...
    if (!message.channel.permissionOverwrites) {
      return message.reply("❌ This channel cannot be unlocked.");
    }

    try {
      // IMPORTANT: use TRUE, not NULL. NULL only removes this specific
      // overwrite and can leave the channel effectively locked by another
      // inherited/explicit deny. TRUE explicitly restores @everyone's
      ...
      // Confirm the overwrite is now an explicit allow before reporting success.
      const everyoneOverwrite =
        message.channel.permissionOverwrites.cache.get(message.guild.roles.everyone.id);

      if (!everyoneOverwrite || !everyoneOverwrite.allow.has(PermissionFlagsBits.SendMessages)) {
        throw new Error("Discord did not apply the @everyone SendMessages allow.");
      }
      ...
      return message.reply(
        "❌ I couldn't unlock this channel. Make sure the bot has **Manage Channels** permission and its role is high enough to manage this channel."
      );
    }
  }
