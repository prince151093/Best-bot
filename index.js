...
const { setGarageRoleId, getGarageRoleId } = require("./server-settings");
...
client.on(
  ...
  async message => {
    ...
    if (lowerContent.startsWith("?setgaragerole")) {
      if (!manageGuild) return message.reply("❌ You need **Manage Server** permission.");
      const role = message.mentions.roles.first();
      if (!role) return message.reply("❌ Use `?setgaragerole @role`");
      try {
        await setGarageRoleId(message.guild.id, role.id);
        return message.reply(`✅ Garage/competition role set to ${role}.

All Event, Season and Championship announcements will tag this role, and registration buttons will be available only to members with this role.`);
      } catch (err) {
        console.error("setgaragerole error:", err);
        return message.reply("❌ I couldn't save that role. Run the latest `competitive-schema.sql` in Supabase and try again.");
      }
    }

    if (lowerContent === "?garagerole") {
      if (!manageGuild) return message.reply("❌ You need **Manage Server** permission.");
      const roleId = await getGarageRoleId(message.guild.id).catch(() => null);
      return message.reply(roleId ? `🎯 Current Garage Role: <@&${roleId}>` : "🎯 No Garage Role is configured. Use `?setgaragerole @role`.");
    }
