...
function fitText(ctx, text, maxWidth, startSize = 32, minSize = 18) {
  let size = startSize;
...
function drawBannerName(ctx, name, centerX, y, maxWidth) {
  const text = `@${String(name || "user").replace(/^@+/, "")}`;

  const size = fitText(ctx, text, maxWidth, 32, 18);
  ctx.font = `700 ${size}px Arial`;
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";

  // Clean, readable name treatment that stays inside the blank name plate.
  ctx.shadowColor = "rgba(0, 145, 255, 0.65)";
  ctx.shadowBlur = 8;
...
async function createSpecialWelcomeBanner(commandMember, newMember) {
  ...
  // Coordinates are for the 1353x698 clean template:
  // left PFP = command user, right PFP = latest joined member.
  drawCircularAvatar(ctx, commandAvatar, 177, 307, 91);
  drawCircularAvatar(ctx, newMemberAvatar, 1153, 307, 91);

  // Put names only in the dedicated bottom name plates.
  drawBannerName(ctx, commandMember.displayName, 260, 496, 245);
  drawBannerName(ctx, newMember.displayName, 1100, 496, 245);
