...
async function checkUnlocks(guild, userId) {
  ...
  if (hours < Number(next.vcHours || 0) || Number(user.messages || 0) < Number(next.messages || 0)) {
    return;
  }

  // IMPORTANT: unlock exactly ONE vehicle per progression check. The previous
  // while-loop could unlock the entire collection in one pass.
  const unlockIndex = index + 1;

  try {
    // Grant first, then advance the legacy progression index. This prevents
    // a failed database/module call from permanently skipping a vehicle.
    await vehicleGame.grantProgressionVehicle(guild.id, userId, unlockIndex);
    index = unlockIndex;
