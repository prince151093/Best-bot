...
client.on(
  ...
  async message => {
    ...
    const specialWelcomeResult = await specialWelcome.handleMessage(message);
    if (specialWelcomeResult !== false) {
      return;
    }
    ...
    const competitiveCommandResult = await competitive.handleCommand(message);
    if (competitiveCommandResult !== false) {
      return;
    }
    ...
    /* =========================
       LOCKED VOICE CHANNEL COMMANDS
       ?lockvc / ?unlockvc / ?accesvc @user / ?removevc @user / ?vcaccess
    ========================= */

    if (lowerContent === "?lockvc" || lowerContent.startsWith("?lockvc ")) {
      if (!message.member.permissions.has(PermissionFlagsBits.ManageRoles)) {
        return message.reply("❌ You need **Manage Roles** permission to lock a voice channel.");
      }

      const voiceChannel = message.member.voice.channel;
      if (!voiceChannel) {
        return message.reply("❌ You must be inside a voice channel to use `?lockvc`.");
      }

      const existing = vcLock.getLock(message.guild.id);
      if (existing) {
        if (existing.channelId === voiceChannel.id) {
          return message.reply("🔒 This voice channel is already locked.");
        }
        return message.reply(`❌ Another voice channel is already locked: <#${existing.channelId}>. Unlock it first.`);
      }

      const allowed = voiceChannel.members
        .filter(member => !member.user.bot)
        .map(member => member.id);

      vcLock.lockChannel(message.guild.id, voiceChannel.id, allowed, message.author.id);

      return message.channel.send({
        content: `🔒 **Voice Channel Locked**\n\n🔊 ${voiceChannel}\n👥 **${allowed.length}** current members have access.\n➕ Use \`?accesvc @user\` to allow someone else.\n🔓 Use \`?unlockvc\` to unlock it.`,
        allowedMentions: { parse: [] }
      });
    }

    if (lowerContent === "?unlockvc" || lowerContent.startsWith("?unlockvc ")) {
      if (!message.member.permissions.has(PermissionFlagsBits.ManageRoles)) {
        return message.reply("❌ You need **Manage Roles** permission to unlock a voice channel.");
      }

      const voiceChannel = message.member.voice.channel;
      if (!voiceChannel) {
        return message.reply("❌ You must be inside the locked voice channel to use `?unlockvc`.");
      }

      if (!vcLock.isLocked(message.guild.id, voiceChannel.id)) {
        return message.reply("❌ This voice channel is not locked.");
      }

      vcLock.unlockChannel(message.guild.id, voiceChannel.id);
      return message.channel.send(`🔓 **${voiceChannel.name}** is now unlocked. Anyone with normal channel permission can join.`);
    }

    if (lowerContent.startsWith("?accesvc")) {
      if (!message.member.permissions.has(PermissionFlagsBits.ManageRoles)) {
        return message.reply("❌ You need **Manage Roles** permission to grant VC access.");
      }

      const voiceChannel = message.member.voice.channel;
      if (!voiceChannel) {
        return message.reply("❌ You must be inside the locked voice channel.");
      }

      if (!vcLock.isLocked(message.guild.id, voiceChannel.id)) {
        return message.reply("❌ This voice channel is not locked. Use `?lockvc` first.");
      }

      const target = message.mentions.members.first();
      if (!target) return message.reply("❌ Use `?accesvc @user`.");
      if (target.user.bot) return message.reply("❌ Bots cannot be added to the VC access list.");

      vcLock.grantAccess(message.guild.id, voiceChannel.id, target.id);
      return message.channel.send(`✅ **${target.displayName}** now has access to **${voiceChannel.name}**.`);
    }

    if (lowerContent.startsWith("?removevc")) {
      if (!message.member.permissions.has(PermissionFlagsBits.ManageRoles)) {
        return message.reply("❌ You need **Manage Roles** permission to remove VC access.");
      }
      ...
      const lock = vcLock.getLock(message.guild.id);
      if (lock?.ownerId === target.id) {
        return message.reply("❌ The person who locked the VC cannot be removed from its access list.");
      }

      vcLock.revokeAccess(message.guild.id, voiceChannel.id, target.id);

      if (target.voice.channelId === voiceChannel.id) {
        try {
          if (message.guild.members.me?.permissions.has(PermissionFlagsBits.MoveMembers)) {
            await target.voice.disconnect("Removed from locked VC access");
          }
        } catch (err) {
          console.error("VC access disconnect error:", err);
        }
      }

      return message.channel.send(`🚫 **${target.displayName}** no longer has access to **${voiceChannel.name}**.`);
    }

    if (lowerContent === "?vcaccess" || lowerContent.startsWith("?vcaccess ")) {
      if (!message.member.permissions.has(PermissionFlagsBits.ManageRoles)) {
        return message.reply("❌ You need **Manage Roles** permission to view VC access.");
      }

      const lock = vcLock.getLock(message.guild.id);
      if (!lock) return message.reply("🔓 No voice channel is currently locked.");

      const names = lock.allowed
        ? [...lock.allowed].map(id => {
    ...
    // Moderation commands: ?kick @user [reason] and ?ban @user [reason]
    if (lowerContent.startsWith("?kick") || lowerContent.startsWith("?ban")) {
      const action = lowerContent.startsWith("?ban") ? "ban" : "kick";
      const requiredPermission = action === "ban" ? PermissionFlagsBits.BanMembers : PermissionFlagsBits.KickMembers;
      const botPermission = action === "ban" ? PermissionFlagsBits.BanMembers : PermissionFlagsBits.KickMembers;

      if (!message.member.permissions.has(requiredPermission)) {
        return message.reply(`❌ You need **${action === "ban" ? "Ban Members" : "Kick Members"}** permission.`);
      }
      if (!message.guild.members.me?.permissions.has(botPermission)) {
        return message.reply(`❌ I need **${action === "ban" ? "Ban Members" : "Kick Members"}** permission.`);
      }

      const target = message.mentions.members.first();
      if (!target) return message.reply(`❌ Use \`?${action} @user [reason]\`.`);
      if (target.id === message.author.id) return message.reply("❌ You cannot moderate yourself.");
      if (target.id === message.guild.ownerId) return message.reply("❌ You cannot moderate the server owner.");
      if (target.user.bot) return message.reply("❌ You cannot moderate bots with this command.");
      ...
      if (message.member.id !== message.guild.ownerId && target.roles.highest.position >= message.member.roles.highest.position) {
        return message.reply("❌ You can only moderate members below your highest role.");
      }
      ...
      try {
        ...
        return message.reply(`✅ ${target.user.tag} was **${action === "ban" ? "banned" : "kicked"}**.\n📝 Reason: ${reason}`);
      } catch (err) {
        console.error(`${action} command error:`, err);
        return message.reply(`❌ I couldn't ${action} that member. Check my permissions and role position.`);
      }
    }
...
async function refreshProfileMessage(
  ...
  try {
    ...
    const profileButton = message.components?.flatMap(r => r.components || []).find(c => String(c.customId || '').startsWith('profile:'));
    const parts = String(profileButton?.customId || '').split(':');
    const pageNo = Number(parts[3] || 0);
    const page = profilePage(member, user, pageNo);
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(`profile:prev:${userId}:${page.page}`).setLabel("Previous").setEmoji("⬅️").setStyle(ButtonStyle.Secondary).setDisabled(page.page <= 0),
      new ButtonBuilder().setCustomId(`profile:next:${userId}:${page.page}`).setLabel("Next").setEmoji("➡️").setStyle(ButtonStyle.Primary).setDisabled(page.page >= page.pageCount - 1)
    );
    await message.edit({ embeds: [page.embed], files: page.files, components: [row] });
...
client.on("guildMemberAdd", async member => {
  ...
  try {
    await specialWelcome.notifyNewMember(member);
  } catch (err) {
...
client.on(
  ...
  async (
    ...
    try {
      ...
      if (joined || enteredDifferentChannel) {

        // If the destination VC is locked, only users already on the access
        // list may enter. Users who are not allowed are disconnected.
        if (vcLock.isLocked(guildId, newState.channelId) && !vcLock.hasAccess(guildId, newState.channelId, userId)) {
          if (member && newState.channelId) {
            try {
              if (!newState.guild.members.me?.permissions.has(PermissionFlagsBits.MoveMembers)) {
                console.warn(`Cannot enforce locked VC ${newState.channelId}: missing Move Members permission.`);
              } else {
                await newState.disconnect("Voice channel is locked");
              }
            } catch (err) {
              console.error("Locked VC disconnect error:", err);
            }
          }
          return;
        }
...
async function attachGameData(guildId, userId, legacyUser) {
  const [gameProfile, gameOwned] = await Promise.all([
    vehicleGame.profile(guildId, userId),
    vehicleGame.vehiclesOf(guildId, userId)
  ]);
  legacyUser.gameProfile = gameProfile;
  legacyUser.gameOwned = gameOwned;
  return legacyUser;
}
...
client.on(
  ...
  async interaction => {
    ...
    try {
      ...
      // Button interactions may arrive in DMs. Game challenges (race, bet,
      // trade, sell confirmation) are intentionally private, so they MUST
      // be processed even when interaction.guild is null.
      if (interaction.isButton() && !interaction.guild) {
        const gameButtonResult = await vehicleGame.handleButton(interaction);
        if (gameButtonResult !== false) return;
        return;
      }
      ...
      if (interaction.isButton()) {
        const competitiveButtonResult = await competitive.handleButton(interaction);
        if (competitiveButtonResult !== false) {
          return;
        }
      ...
      if (interaction.isButton() && interaction.customId.startsWith("profile:")) {
        ...
        if (interaction.user.id !== ownerId) return interaction.reply({ content: "❌ Only the person who opened this profile can use these buttons.", ephemeral: true });
        const currentPage = Number(pageText) || 0;
        const nextPage = direction === "next" ? currentPage + 1 : currentPage - 1;
        const freshUser = getUser(ownerId, interaction.guild.id);
        await attachGameData(interaction.guild.id, ownerId, freshUser);
        ...
        const page = profilePage(targetMember, freshUser, nextPage);
        const row = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId(`profile:prev:${ownerId}:${page.page}`).setLabel("Previous").setEmoji("⬅️").setStyle(ButtonStyle.Secondary).setDisabled(page.page <= 0),
          new ButtonBuilder().setCustomId(`profile:next:${ownerId}:${page.page}`).setLabel("Next").setEmoji("➡️").setStyle(ButtonStyle.Primary).setDisabled(page.page >= page.pageCount - 1)
        );
        const menuRow = new ActionRowBuilder().addComponents(
          ...
          new ButtonBuilder().setCustomId('vlmenu:main').setLabel('Menu').setEmoji('📋').setStyle(ButtonStyle.Secondary)
        );
        return interaction.update({ embeds: [page.embed], files: page.files, components: [row, menuRow] });
      }
      ...
      if (
        ...
        await attachGameData(interaction.guild.id, targetId, targetUser);
        const page =
          garagePage(
            targetMember,
            targetUser,
      ...
      if (
        ...
        await attachGameData(interaction.guild.id, interaction.user.id, freshUser);
        const page =
          garagePage(
            interaction.member,
            freshUser,
      ...
      if (interaction.commandName === "kick" || interaction.commandName === "ban") {
        ...
        const requiredPermission = action === "ban" ? PermissionFlagsBits.BanMembers : PermissionFlagsBits.KickMembers;
        const botPermission = requiredPermission;
        ...
        if (!interaction.member.permissions.has(requiredPermission)) {
          return interaction.editReply({ content: `❌ You need **${action === "ban" ? "Ban Members" : "Kick Members"}** permission.` });
        }
        if (!interaction.guild.members.me?.permissions.has(botPermission)) {
          return interaction.editReply({ content: `❌ I need **${action === "ban" ? "Ban Members" : "Kick Members"}** permission.` });
        }
        if (!target) return interaction.editReply({ content: "❌ That user is not a member of this server." });
        if (target.id === interaction.user.id) return interaction.editReply({ content: "❌ You cannot moderate yourself." });
        if (target.id === interaction.guild.ownerId) return interaction.editReply({ content: "❌ You cannot moderate the server owner." });
        if (target.user.bot) return interaction.editReply({ content: "❌ You cannot moderate bots with this command." });
        if (!target.manageable) return interaction.editReply({ content: "❌ I cannot moderate that member because of role hierarchy." });
        if (interaction.user.id !== interaction.guild.ownerId && target.roles.highest.position >= interaction.member.roles.highest.position) {
          return interaction.editReply({ content: "❌ You can only moderate members below your highest role." });
        }

        try {
          if (action === "ban") await target.ban({ reason });
          else await target.kick(reason);
          return interaction.editReply({ content: `✅ ${targetUser.tag} was **${action === "ban" ? "banned" : "kicked"}**.\n📝 Reason: ${reason}` });
        } catch (err) {
          console.error(`${action} slash command error:`, err);
          return interaction.editReply({ content: `❌ I couldn't ${action} that member. Check my permissions and role position.` });
        }
      }
      ...
      if (
        ...
        await attachGameData(interaction.guild.id, interaction.user.id, user);
        const page = profilePage(interaction.member, user, 0);
        const row = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId(`profile:prev:${interaction.user.id}:0`).setLabel("Previous").setEmoji("⬅️").setStyle(ButtonStyle.Secondary).setDisabled(page.page <= 0),
          new ButtonBuilder().setCustomId(`profile:next:${interaction.user.id}:0`).setLabel("Next").setEmoji("➡️").setStyle(ButtonStyle.Primary).setDisabled(page.page >= page.pageCount - 1)
        );
        const menuRow = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId('vlmenu:garage').setLabel('Garage').setEmoji('🏠').setStyle(ButtonStyle.Primary),
          new ButtonBuilder().setCustomId('vlmenu:dealership').setLabel('Dealership').setEmoji('🚗').setStyle(ButtonStyle.Success),
        ...
        await interaction.editReply({ embeds: [page.embed], files: page.files, components: [row, menuRow] });
      ...
      if (
        ...
        await attachGameData(interaction.guild.id, targetUser.id, targetData);
        const page =
          garagePage(
            targetMember,
            targetData,
...
client.once('ready', async () => {
  ...
  try { await competitive.startCompetitiveAutomation(client); console.log('Competitive season automation started.'); }
  catch (err) { console.error('Competitive season automation startup failed:', err); }
