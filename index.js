...
function getMediaOnlyChannelId(guildId) {
  const settings = channelShortcuts[guildId];
  return settings && typeof settings === "object" ? settings.__mediaOnlyChannelId || null : null;
}

function setMediaOnlyChannel(guildId, channelId) {
  if (!channelShortcuts[guildId]) channelShortcuts[guildId] = {};
  channelShortcuts[guildId].__mediaOnlyChannelId = channelId;
  saveChannelShortcuts();
}

function isAllowedMediaMessage(message) {
  // Any attachment is treated as media, including images, videos, GIFs,
  // audio, and uploaded files. Stickers are also allowed.
  return message.attachments?.size > 0 || message.stickers?.size > 0;
}

async function enforceMediaOnlyChannel(message) {
  if (!message.guild || message.author?.bot) return false;
  const mediaChannelId = getMediaOnlyChannelId(message.guild.id);
  if (!mediaChannelId || message.channel.id !== mediaChannelId) return false;
  if (isAllowedMediaMessage(message)) return false;

  try {
    await message.delete();
  } catch (err) {
    console.error("Media-only message delete error:", err);
  }

  try {
    const warning = await message.channel.send(
      `${message.author} ⚠️ **Media-only channel!** Please send an image, GIF, video, sticker, or file instead of a text message.`
    );
    setTimeout(() => {
      warning.delete().catch(() => {});
    }, 5000);
  } catch (err) {
    console.error("Media-only warning send error:", err);
  }
  return true;
}
...
async function handleActivityCommand(message) {
  ...
  if (lower === "?lock" || lower === "?unlock") {
    ...
    if (!me?.permissionsIn(message.channel).has(PermissionFlagsBits.ManageChannels)) {
      return message.reply("❌ I need **Manage Channels** permission in this channel.");
    }

    if (!message.channel.permissionOverwrites?.edit) {
      return message.reply("❌ This channel does not support permission locking.");
    }
    ...
    try {
      // Use an explicit allow on unlock. Using null only removes the overwrite;
      // it does not guarantee that members can send when another inherited or
      // role/member overwrite is denying SendMessages.
      await message.channel.permissionOverwrites.edit(everyone, {
        SendMessages: locking ? false : true
      }, { reason: locking ? `Channel locked by ${message.author.tag}` : `Channel unlocked by ${message.author.tag}` });

      // Read the overwrite back so the command never reports success before
      // Discord has actually stored the requested state.
      const overwrite = message.channel.permissionOverwrites.cache.get(everyone.id);
      const value = overwrite?.deny?.has(PermissionFlagsBits.SendMessages)
        ? false
      ...
      if (locking && value !== false) {
        throw new Error("Discord did not apply the SendMessages deny overwrite.");
      }
      if (!locking && value !== true) {
        throw new Error("Discord did not apply the SendMessages allow overwrite.");
      }

      return message.channel.send(
        locking
          ? "🔒 **Channel locked.** Members can read the channel, but cannot send messages."
          : "🔓 **Channel unlocked.** Members can send messages again."
      );
    } catch (err) {
      console.error(`Channel ${locking ? "lock" : "unlock"} error:`, err);
      return message.reply(
        `❌ I couldn't ${locking ? "lock" : "unlock"} this channel. Check my **Manage Channels** permission and Discord channel permissions.`
      );
    }
  }
...
client.on(
  ...
  async message => {
    ...
    // Configure the current channel as a media-only channel.
    // This command is handled before media enforcement so the channel can
    // be configured even after it has already been made media-only.
    if (message.content.trim().toLowerCase() === "?setasmedia") {
      if (!message.member.permissions.has(PermissionFlagsBits.ManageGuild)) {
        return message.reply("❌ You need **Manage Server** permission.");
      }
      if (!message.channel.isTextBased()) {
        return message.reply("❌ This channel cannot be used as a media-only channel.");
      }
      setMediaOnlyChannel(message.guild.id, message.channel.id);
      return message.reply(
        "✅ This channel is now **media-only**. Text messages will be deleted and the warning will disappear after 5 seconds."
      );
    }

    // Enforce media-only mode before normal commands/game handlers run.
    // Messages containing attachments or stickers are allowed.
    if (await enforceMediaOnlyChannel(message)) {
      return;
    }
