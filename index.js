...
function drawCircularAvatar(ctx, image, x, y, radius) {
  // Discord avatars are rendered as a true circular crop and cover the
  // entire inner avatar area. No stretching and no visible placeholder.
...
function getCleanFont(size) {
  // Keep typography simple and professional: straight, bold, non-italic.
...
function cleanDisplayName(member) {
  return String(member?.displayName || member?.user?.username || "user")
    .replace(/[\r\n]/g, " ")
    .trim();
}
...
function drawPlateName(ctx, member, centerX, y, maxWidth) {
  ...
  const size = fitText(ctx, text, maxWidth, 27, 14);
...
function drawCenteredWelcomeText(ctx, newMember, commandMember, canvasWidth) {
  ...
  // The blank center of the supplied template is intentionally populated at
  // runtime. The artwork itself is never replaced or regenerated.
  const line1 = `${newName} LET'S TALK TOGETHER,`;
  const line2 = `I AM ${commandName}`;
  const maxWidth = 600;
  const size = Math.min(
    fitText(ctx, line1, maxWidth, 34, 20),
    fitText(ctx, line2, maxWidth, 34, 20)
  );
  ...
  ctx.shadowBlur = 5;
  ctx.fillText(line1, canvasWidth / 2, 365);
  ctx.fillText(line2, canvasWidth / 2, 408);
  ctx.restore();
}
...
async function createSpecialWelcomeBanner(commandMember, newMember) {
  ...
  // This is a clean blank template. Only the two PFPs and two name plates
  // are populated at runtime; the artwork and center area are never altered.
  ctx.drawImage(template, 0, 0, canvas.width, canvas.height);

  const [commandAvatar, newMemberAvatar] = await Promise.all([
    fetchAvatarImage(commandMember),
    fetchAvatarImage(newMember)
  ]);

  // Exact positions of the blank areas in the supplied 1353x698 template.
  // The avatar is clipped to the inner circle so it never spills into the
  // neon ring or leaves the placeholder silhouette visible.
  drawCircularAvatar(ctx, commandAvatar, 176, 306, 108);
  drawCircularAvatar(ctx, newMemberAvatar, 1154, 306, 108);

  // Names stay inside the two existing blank name plates.
  drawPlateName(ctx, commandMember, 257, 496, 225);
  drawPlateName(ctx, newMember, 1097, 496, 225);

  // Professional straight white text in the blank center area.
  ...
  async message => {
    ...
    if (lowerContent === "?w") {
      ...
      try {
        ...
        const welcomeText = [
          `<@${latestMember.id}> AGAIN THANKS FROM <@${message.author.id}> FOR JOINING US LET'S DO FUN TOGETHER :love1::love2:`
        ].join("\n");
