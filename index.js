...
async function checkUnlocks(guild, userId) {
  ...
  try {
    ...
    // Advance the progression pointer even if this is a safe duplicate retry.
    index = unlockIndex;
    setVehicleIndex(userId, guild.id, index);
    console.log(`Progression unlock checked: guild=${guild.id} user=${userId} vehicle=${unlockIndex} granted=${result?.granted !== false}`);
  } catch (err) {
