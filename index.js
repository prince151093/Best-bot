...
function drawPlateName(ctx, member, centerX, y, maxWidth) {
  ...
  const size = fitText(ctx, text, maxWidth, 28, 15);
...
function drawCenteredWelcomeText(ctx, newMember, commandMember, canvasWidth) {
  ...
  // Three large, straight, bold lines. Keep the artwork untouched and use
  // only the clean center area between the two avatar panels.
  const lines = [
    `${newName} LET'S`,
    `TALK TOGETHER, I AM`,
    commandName
  ...
  const lineY = [335, 380, 425];
  lines.forEach((line, i) => ctx.fillText(line, canvasWidth / 2, lineY[i]));
  ctx.restore();
}
...
async function createSpecialWelcomeBanner(commandMember, newMember) {
  ...
  drawCircularAvatar(ctx, commandAvatar, 176, 306, 105);
  drawCircularAvatar(ctx, newMemberAvatar, 1154, 306, 105);

  // Keep names inside the actual text portions of the two plates.
  // Left is shifted slightly left; right is shifted slightly right.
  drawPlateName(ctx, commandMember, 245, 496, 220);
  drawPlateName(ctx, newMember, 1110, 496, 220);
