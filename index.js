...
client.on(
  ...
  async message => {
    ...
    // Voice-channel access commands are handled first so other command
    // systems cannot consume or ignore them.
    const vcCommandResult = await vcLock.handleCommand(message, client);
    if (vcCommandResult !== false) return;
  ...
  async (
    ...
    try {
      ...
      if (joined) {
        ...
        // Enforce locked VC access immediately after a user joins.
        if (newState.channelId && vcLock.isLocked(guildId, newState.channelId) && !vcLock.hasAccess(guildId, newState.channelId, userId)) {
          try {
            if (!member?.voice?.disconnect) throw new Error("Member voice state is unavailable.");
            await member.voice.disconnect("VC is locked; user is not on the access list.");
            console.log(`VC LOCK: disconnected ${userId} from ${newState.channelId}`);
          } catch (err) {
            console.error(`VC LOCK: could not disconnect ${userId} from ${newState.channelId}:`, err);
          }
          return;
        }
