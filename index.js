...
/* =========================
   SPECIAL PERSONAL WELCOME
   ?setspecialwelcomechannel -> saves the current channel
   ?w -> welcomes the latest joined member in that channel
========================= */

const SPECIAL_WELCOME_FILE = path.join(__dirname, "special-welcome.json");
const SPECIAL_WELCOME_TEMPLATE = path.join(__dirname, "assets", "special-welcome-template.png");
let specialWelcomeSettings = {};

function loadSpecialWelcomeSettings() {
  try {
    if (fs.existsSync(SPECIAL_WELCOME_FILE)) {
      const data = JSON.parse(fs.readFileSync(SPECIAL_WELCOME_FILE, "utf8"));
      specialWelcomeSettings = data && typeof data === "object" ? data : {};
    }
  } catch (err) {
    console.error("Special welcome settings load error:", err);
    specialWelcomeSettings = {};
  }
}

function saveSpecialWelcomeSettings() {
  try {
    fs.writeFileSync(
      SPECIAL_WELCOME_FILE,
      JSON.stringify(specialWelcomeSettings, null, 2)
    );
  } catch (err) {
    console.error("Special welcome settings save error:", err);
  }
}

loadSpecialWelcomeSettings();

let specialWelcomeTemplatePromise = null;

function getSpecialWelcomeTemplate() {
  if (!specialWelcomeTemplatePromise) {
    specialWelcomeTemplatePromise = loadImage(SPECIAL_WELCOME_TEMPLATE);
  }
  return specialWelcomeTemplatePromise;
}
...
async function fetchAvatarImage(member) {
  ...
  // Remove large white/empty borders from avatars before the circular mask.
  const probe = createCanvas(image.width, image.height);
  const pctx = probe.getContext("2d");
  pctx.drawImage(image, 0, 0, image.width, image.height);
  ...
  const side = Math.min(image.width, Math.max(w, h));
  ...
  const cropped = createCanvas(side, side);
  cropped.getContext("2d").drawImage(image, x, y, side, side, 0, 0, side, side);
  return cropped;
}
...
function drawCircularAvatar(ctx, image, x, y, radius) {
  ...
  ctx.clip();
  ctx.drawImage(image, 0, 0, image.width, image.height, x - radius, y - radius, size, size);
  ctx.restore();
}

function getCleanFont(size) {
  // Use a broad Unicode-capable font stack so Discord display names that
  // contain styled Unicode, non-Latin scripts, or symbols render instead of
  // turning into missing-glyph boxes. The bold weight keeps the template
  // typography consistent without forcing a decorative/italic style.
  return `700 ${size}px "Noto Sans", "Noto Sans Display", "DejaVu Sans", Arial, sans-serif`;
}

function fitText(ctx, text, maxWidth, startSize = 30, minSize = 14) {
  let size = startSize;
  while (size >= minSize) {
    ctx.font = getCleanFont(size);
    if (ctx.measureText(text).width <= maxWidth) return size;
    size -= 1;
  }
  ctx.font = getCleanFont(minSize);
  return minSize;
}

function cleanDisplayName(member) {
  return String(member?.displayName || member?.user?.username || "user").replace(/[\r\n]/g, " ").trim();
}

function drawPlateName(ctx, member, centerX, y, maxWidth) {
  const text = `@${cleanDisplayName(member).replace(/^@+/, "")}`;
  const size = fitText(ctx, text, maxWidth, 25, 13);
  ctx.save();
  ctx.font = getCleanFont(size);
...
function drawCenteredWelcomeText(ctx, newMember, commandMember, canvasWidth) {
  const newName = `@${cleanDisplayName(newMember).replace(/^@+/, "")}`;
  const commandName = `@${cleanDisplayName(commandMember).replace(/^@+/, "")}`;
  const lines = [newName, "LET'S TALK TOGETHER", `I AM ${commandName}`];
  const maxWidth = 680;
  const sizes = lines.map((line, i) => fitText(ctx, line, maxWidth, i === 1 ? 44 : 42, 25));
  ...
  ctx.shadowColor = "rgba(0,0,0,0.98)";
  ctx.shadowBlur = 6;
  [365, 420, 475].forEach((y, i) => ctx.fillText(lines[i], canvasWidth / 2, y));
  ctx.restore();
}

async function createSpecialWelcomeBanner(commandMember, newMember) {
  const template = await getSpecialWelcomeTemplate();
  const canvas = createCanvas(template.width, template.height);
  const ctx = canvas.getContext("2d");
  ctx.drawImage(template, 0, 0, canvas.width, canvas.height);
  const [commandAvatar, newMemberAvatar] = await Promise.all([fetchAvatarImage(commandMember), fetchAvatarImage(newMember)]);

  // Exact inner PFP circles of the supplied blank template.
  drawCircularAvatar(ctx, commandAvatar, 177, 306, 112);
  drawCircularAvatar(ctx, newMemberAvatar, 1154, 306, 112);

  // Text stays inside each name plate and never crosses the Discord icon.
  drawPlateName(ctx, commandMember, 257, 495, 245);
  drawPlateName(ctx, newMember, 1200, 495, 245);
  drawCenteredWelcomeText(ctx, newMember, commandMember, canvas.width);
...
client.on(
  ...
  async message => {
    ...
    // Special personal welcome channel: only the current channel is saved.
    if (lowerContent === "?setspecialwelcomechannel") {
      if (!manageGuild) return message.reply("❌ You need **Manage Server** permission.");
      if (!message.channel.isTextBased()) return message.reply("❌ This channel cannot be used for the special welcome.");

      specialWelcomeSettings[message.guild.id] = {
        channelId: message.channel.id
      };
      saveSpecialWelcomeSettings();

      return message.reply(
        `✅ Special welcome channel set to ${message.channel}.\nOnly this channel can use \`?w\`.`
      );
    }

    if (lowerContent === "?w") {
      const configuredChannelId = specialWelcomeSettings[message.guild.id]?.channelId;

      if (!configuredChannelId) {
        return message.reply("❌ No special welcome channel is set. Use `?setspecialwelcomechannel` first.");
      }

      if (message.channel.id !== configuredChannelId) {
        return message.reply(`❌ Use \`?w\` only in <#${configuredChannelId}>.`);
      }

      try {
        // Fetch the guild member list so the latest join timestamp is reliable
        // even after a bot restart.
        await message.guild.members.fetch();

        const latestMember = message.guild.members.cache
          .filter(member => Number.isFinite(member.joinedTimestamp))
          .sort((a, b) => b.joinedTimestamp - a.joinedTimestamp)
          .first();

        if (!latestMember) {
          return message.reply("❌ I couldn't find a recently joined member.");
        }

        const banner = await createSpecialWelcomeBanner(message.member, latestMember);

        const attachment = new AttachmentBuilder(banner, {
          name: "vehicle-life-special-welcome.png"
        });

        // Keep the exact supplied banner design and put the custom message
        // below the image as a normal Discord message.
        const welcomeText =
          `<@${latestMember.id}> WELCOME FROM <@${message.author.id}> FOR JOINING US LET'S DO FUN TOGETHER`;
        ...
        console.error("Special welcome error:", err);
        return message.reply("❌ I couldn't generate the special welcome banner. Check my permissions and try again.");
      }
