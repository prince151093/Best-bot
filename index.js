...
client.on(
  ...
  async message => {
    ...
    // Move all eligible members from other voice channels into the command author's current voice channel.
    if (lowerContent === "?dragall") {
      ...
      if (!botMember || !sourceChannel.permissionsFor(botMember)?.has(PermissionFlagsBits.MoveMembers)) {
        return message.reply("❌ I need the **Move Members** permission in your voice channel.");
      }
