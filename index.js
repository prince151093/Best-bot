...
client.on(
  ...
  async message => {
    ...
    // Move all eligible members from other voice channels into the command author's current VC.
    if (lowerContent === "?dragall") {
      if (!message.member.permissions.has(PermissionFlagsBits.Administrator)) {
        return message.reply("❌ You need **Administrator** permission to use `?dragall`.");
      }

      const sourceChannel = message.member.voice.channel;
      if (!sourceChannel || !sourceChannel.isVoiceBased()) {
        return message.reply("❌ Join a voice channel first, then use `?dragall`.");
      }

      if (!sourceChannel.permissionsFor(message.guild.members.me)?.has(PermissionFlagsBits.MoveMembers)) {
        return message.reply("❌ I need the **Move Members** permission to do that.");
      }

      const movableMembers = message.guild.voiceStates.cache
        .filter(state => state.channelId && state.channelId !== sourceChannel.id)
        .map(state => state.member)
        .filter(member => member && !member.user.bot && sourceChannel.permissionsFor(message.guild.members.me)?.has(PermissionFlagsBits.MoveMembers, member));

      if (!movableMembers.length) {
        return message.reply("ℹ️ No eligible members found in other voice channels.");
      }
      ...
      for (const member of movableMembers) {
        try {
          await member.voice.setChannel(sourceChannel, "Requested by ?dragall");
          moved++;
        } catch (err) {
          failed++;
      ...
      return message.reply(`✅ **Dragall complete!** Moved **${moved}** member(s) to ${sourceChannel}.${failed ? `\n⚠️ Failed: **${failed}**` : ""}`);
    }
...
client.on(
  ...
  async interaction => {
    ...
    try {
      ...
      // Load profile data only for commands that actually need it.
      // This prevents unrelated commands such as /topgarages and /setup
      // from failing when a profile/ownership lookup has an issue.
      let user = null;
      const loadUser = async () => {
        if (!user) {
          user = await getCardUser(
            interaction.guild.id,
            interaction.user.id
          );
        }
