...
async function getDisplayUser(guildId, userId) {
  const [p, owned] = await Promise.all([profile(guildId, userId), vehiclesOf(guildId, userId)]);
  const legacy = getUser(userId, guildId) || {};
  return {
    ...legacy,
    ...p,
    ownedVehicleIds: owned.map(row => Number(row.vehicle_id)).filter(Number.isInteger),
    ownedVehicles: owned
  };
}
