...
const { memberHasGarageRole } = require('./server-settings');
...
async function ensureLiveWorld(guildId) {
  ...
  const open = await sb.from('events').select('id,entry_fee').eq('guild_id', guildId).eq('status', 'open');
  if (open.error) throw open.error;

  // All Vehicle Life events are FREE to enter. Normalize older open events
  // that may still have the legacy ₹5,000 entry fee stored in the database.
  for (const event of open.data || []) {
    if (Number(event.entry_fee || 0) !== 0) {
      const normalized = await sb.from('events').update({ entry_fee: 0 }).eq('id', event.id).eq('guild_id', guildId);
  ...
  if (!open.data?.length) {
    ...
    const ins = await sb.from('events').insert({ guild_id: guildId, name: 'Daily Sprint', description: 'Complete races during the event and earn points. Free entry for everyone.', entry_fee: 0, status: 'open', starts_at: now.toISOString(), ends_at: ends, rewards: { cash: 25000, xp: 150 } });
    if (ins.error) throw ins.error;
  }
  return season;
}
...
async function handle(message) {
  ...
  try {
    ...
    if (cmd === '?events') {
      ...
      if (r.error) throw r.error;
      if (!r.data?.length) return message.reply('🎉 **EVENTS**\n\nNo active events right now.');
      const view = await buildEventPanel(gid, uid, r.data, 0);
      return message.reply(view);
    }
    if (cmd === '?eventjoin') {
      if (!(await memberHasGarageRole(gid, message.member))) return message.reply('❌ Event registration is available only to members with the configured Garage Role.');
      ...
      if (ins.error) throw ins.error;
      return message.reply(`🎉 You joined **${e.data.name}** for **FREE**! Complete races before it closes.`);
    }
...
async function buildEventPanel(guildId, userId, events, index = 0, mode = 'overview') {
  ...
  const now = Date.now();
  const remaining = Math.max(0, ends.getTime() - now);
  const hours = Math.floor(remaining / 3600000);
  const mins = Math.floor((remaining % 3600000) / 60000);
  const rewardCash = Number(e.rewards?.cash || 0);
  const rewardXp = Number(e.rewards?.xp || 0);

  const countR = await sb.from('event_entries').select('id', { count: 'exact', head: true }).eq('event_id', e.id);
  if (countR.error) throw countR.error;
  const mineR = await sb.from('event_entries').select('id,score,rewarded').eq('event_id', e.id).eq('user_id', userId).maybeSingle();
  if (mineR.error) throw mineR.error;
  const participantCount = Number(countR.count || 0);
  const joined = !!mineR.data;

  const embed = new EmbedBuilder()
    .setTitle(`🎉 ${e.name}`)
    .setDescription(e.description || 'Compete in races and climb the event leaderboard.')
    .addFields(
      { name: '🎟️ Entry', value: '**FREE**', inline: true },
      { name: '🏆 Prize Pool', value: `${rewardCash > 0 ? money(rewardCash) : 'No cash prize'}${rewardXp > 0 ? `\\n+ ${rewardXp} XP` : ''}`, inline: true },
      { name: '👥 Players', value: `${participantCount}`, inline: true },
      { name: '🕒 Starts', value: starts.toLocaleString('en-IN'), inline: true },
      { name: '⏳ Ends', value: ends.toLocaleString('en-IN'), inline: true },
      { name: '⏱️ Remaining', value: remaining > 0 ? `${hours}h ${mins}m` : 'Ending now', inline: true }
    )
    .setFooter({ text: `Event #${e.id} • ${safeIndex + 1}/${events.length} • Free entry` });

  if (mode === 'leaderboard') {
    const lb = await sb.from('event_entries').select('user_id,score').eq('event_id', e.id).order('score', { ascending: false }).limit(10);
    if (lb.error) throw lb.error;
    const lines = (lb.data || []).map((x, i) => `${i < 3 ? ['🥇','🥈','🥉'][i] : `**#${i + 1}**`} <@${x.user_id}> — **${Number(x.score || 0)} pts**`);
    embed.addFields({ name: '🏆 Leaderboard', value: lines.join('\\n') || 'No players have joined yet.' });
  } else if (mode === 'details') {
    embed.addFields(
      { name: '📜 How it works', value: 'Join for free, complete eligible races while the event is active, and earn event points automatically.' },
      { name: '💰 Prize distribution', value: rewardCash > 0 ? `🥇 **1st place:** ${money(rewardCash)}${rewardXp > 0 ? ` + ${rewardXp} XP` : ''}` : 'No cash prize configured.' },
      { name: '👤 Your status', value: joined ? `✅ Joined\\n📊 Score: **${Number(mineR.data.score || 0)} pts**` : '❌ Not joined' }
    );
  } else {
    embed.addFields({ name: '👤 Your Status', value: joined ? `✅ Joined • **${Number(mineR.data.score || 0)} pts**` : '❌ Not joined yet' });
  }

  const row1 = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(`event:join:${e.id}`).setLabel(joined ? 'Joined' : 'Join Event').setEmoji('🎮').setStyle(ButtonStyle.Success).setDisabled(joined),
    new ButtonBuilder().setCustomId(`event:details:${e.id}`).setLabel('Details').setEmoji('ℹ️').setStyle(ButtonStyle.Primary),
    new ButtonBuilder().setCustomId(`event:leaderboard:${e.id}`).setLabel('Leaderboard').setEmoji('🏆').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId(`event:refresh:${safeIndex}`).setLabel('Refresh').setEmoji('🔄').setStyle(ButtonStyle.Secondary)
  );
  const row2 = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(`event:prev:${safeIndex}`).setLabel('◀ Previous').setStyle(ButtonStyle.Secondary).setDisabled(safeIndex <= 0),
    new ButtonBuilder().setCustomId(`event:next:${safeIndex}`).setLabel('Next ▶').setStyle(ButtonStyle.Secondary).setDisabled(safeIndex >= events.length - 1)
  );
...
async function handleButton(interaction) {
  ...
  try {
    if (interaction.customId.startsWith('event:')) {
      ...
      await ensureLiveWorld(guildId);
      const eventsR = await sb.from('events').select('*').eq('guild_id', guildId).eq('status', 'open').order('starts_at');
      if (eventsR.error) throw eventsR.error;
      const events = eventsR.data || [];
      if (!events.length) return interaction.update({ content: '🎉 **No active events right now.**', embeds: [], components: [] });
      // Scheduled announcement buttons use the special `daily` value and
      // always open the first currently active event.
      if (value === 'daily') value = String(events[0].id);
      let index = Number(value) || 0;
      if (action === 'join') {
        if (!(await memberHasGarageRole(guildId, interaction.member))) {
          return interaction.reply({ content: '❌ Event registration is available only to members with the configured Garage Role.', ephemeral: true });
        }
        const eventId = Number(value);
        const e = events.find(x => Number(x.id) === eventId);
        if (!e) return interaction.reply({ content: '❌ Event not found or closed.', ephemeral: true });
        const existing = await sb.from('event_entries').select('id').eq('event_id', eventId).eq('user_id', userId).maybeSingle();
        if (existing.error) throw existing.error;
        if (existing.data) return interaction.reply({ content: '⚠️ You are already registered for this event.', ephemeral: true });
        const ins = await sb.from('event_entries').insert({ event_id: eventId, guild_id: guildId, user_id: userId, score: 0, joined_at: new Date().toISOString() });
        if (ins.error) throw ins.error;
        const view = await buildEventPanel(guildId, userId, events, events.findIndex(x => Number(x.id) === eventId));
        return interaction.update(view);
      }
      if (action === 'prev' || action === 'next' || action === 'refresh') {
        if (action === 'prev') index = Math.max(0, index - 1);
        if (action === 'next') index = Math.min(events.length - 1, index + 1);
        const view = await buildEventPanel(guildId, userId, events, index);
        return interaction.update(view);
      }
      const eventId = Number(value);
      const idx = events.findIndex(x => Number(x.id) === eventId);
      if (idx < 0) return interaction.reply({ content: '❌ Event is no longer active.', ephemeral: true });
      const view = await buildEventPanel(guildId, userId, events, idx, action);
      return interaction.update(view);
    }
