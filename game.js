...
const { resetUserProgress } = require('./db');
...
async function grantProgressionVehicle(guildId,userId,index){
  ...
  const r=await sb.from('user_vehicles').insert({guild_id:guildId,user_id:userId,vehicle_id:v.id,condition:100});
  ...
  try{
    ...
    if(cmd==='?adminreset'){
      ...
      const resetUser=async (userId)=>{
        ...
        // Clear the legacy in-memory cache too. Top Garages and the old
        // progression listeners read this cache, so leaving it populated
        // would make a successful database reset appear to have failed.
...
module.exports={handle,handleButton,vehicles:V,startAutomation,grantProgressionVehicle};
