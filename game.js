...
async function handleButton(interaction) {
  ...
  try {
    ...
    if (id.startsWith('vlmenu:')) {
      ...
      if (action === 'garage') {
        const p = await profile(gid, uid); const owned = await vehiclesOf(gid, uid); const best = [...owned].map(o => V[o.vehicle_id - 1]).filter(Boolean).sort((a,b) => classRank(b.class)-classRank(a.class)||b.performance-a.performance)[0];
        const e = profileEmbed(interaction.member, p, owned, best).setTitle(`🏠 ${interaction.member.displayName}'S GARAGE`);
        const first = owned[0] ? V[owned[0].vehicle_id - 1] : null;
        if (first) e.setImage(`attachment://${imageName(first)}`);
        return interaction.editReply({ embeds: [e], files: first ? [{ attachment: vehicleImage(first), name: imageName(first) }] : [], components: first ? vehicleActionRows(first.id, uid) : mainMenuRows() });
      }
      if (action === 'missions') {
    ...
    if (id.startsWith('vehicleaction:')) {
      ...
      if (action === 'active') {
        ...
        return interaction.editReply({ content: `🚗 **${v.name}** is now your active vehicle.`, embeds: [], components: vehicleActionRows(vehicleId, uid) });
      }
      if (action === 'race') {
        ...
        return interaction.editReply({ content: `🏁 **${v.name}** is now your race car.`, embeds: [], components: vehicleActionRows(vehicleId, uid) });
      }
      if (action === 'sell') {
