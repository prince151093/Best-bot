...
async function handle(message) {
  ...
  try {
    ...
    if (cmd === '?sell') {
      ...
      if (Number(p.active_vehicle_id) === Number(v.id) || Number(p.race_vehicle_id) === Number(v.id)) {
        const which = Number(p.race_vehicle_id) === Number(v.id) ? 'race vehicle' : 'active vehicle';
        return message.reply(`❌ **${v.name}** is currently your ${which}. Select another vehicle first, then sell it.`);
      }
...
async function handleButton(interaction) {
  ...
  try {
    ...
    if (interaction.customId.startsWith('racedecline:')) {
      ...
      if (interaction.user.id !== r.to) return interaction.reply({ content: '❌ Only the challenged player can respond to this race.', ephemeral: true });
      pendingRaces.delete(id);
      return interaction.update({ content: '❌ Race challenge declined.', components: [] });
    }
    if (interaction.customId.startsWith('raceaccept:')) {
      const id = interaction.customId.slice(11);
      const r = pendingRaces.get(id);
      if (!r || Date.now() > r.expires) return interaction.reply({ content: '❌ Race challenge expired.', ephemeral: true });
      if (interaction.user.id !== r.to) return interaction.reply({ content: '❌ Only the challenged player can accept this race.', ephemeral: true });
      pendingRaces.delete(id);
      const a = await interaction.guild.members.fetch(r.from).catch(() => null);
      const b = await interaction.guild.members.fetch(r.to).catch(() => null);
      if (!a || !b) return interaction.update({ content: '❌ One of the players is no longer in the server.', components: [] });
      const result = await runRace(interaction.guild, a, r.type, b);
      return interaction.update({ content: `🏁 **RACE ACCEPTED**\n${raceText(r.type, result)}\n\n👤 Challenger: <@${r.from}>\n👤 Accepted by: <@${r.to}>`, components: [] });
    }
