...
async function ensureDailyMissions(guildId, userId) {
  const period = todayKey();
  const existing = await sb.from('missions')
    .select('mission_key')
    .eq('guild_id', guildId)
    .eq('user_id', userId)
    .eq('period_key', period);
  if (existing.error) throw existing.error;
  const have = new Set((existing.data || []).map(x => x.mission_key));
  const missing = MISSION_DEFS
    .filter(([mission_key]) => !have.has(mission_key))
    .map(([mission_key, , target, reward]) => ({
      guild_id: guildId, user_id: userId, mission_key, progress: 0, target, reward, period_key: period, completed: false
    }));
  if (!missing.length) return;
  const ins = await sb.from('missions').upsert(missing, {
    onConflict: 'guild_id,user_id,mission_key,period_key',
...
async function topGarages(guildId, limit = 10) {
  const [profiles, ownedR] = await Promise.all([
    sb.from('game_profiles').select('user_id,balance,driver_xp,driver_level,active_vehicle_id').eq('guild_id', guildId),
    sb.from('user_vehicles').select('*').eq('guild_id', guildId)
  ]);
  if (profiles.error) throw profiles.error;
  if (ownedR.error) throw ownedR.error;
  const byUser = new Map();
  for (const row of ownedR.data || []) {
    const owned = byUser.get(row.user_id) || [];
    owned.push(normalizeVehicleRow(row));
    byUser.set(row.user_id, owned);
  }
  const all = [];
  for (const p of profiles.data || []) {
    const owned = byUser.get(p.user_id) || [];
    if (!owned.length) continue;
    const best = owned.map(o => V[o.vehicle_id - 1]).filter(Boolean)
      .sort((a, b) => classRank(b.class) - classRank(a.class) || b.performance - a.performance)[0];
