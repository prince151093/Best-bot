...
async function changeBalance(guildId, userId, delta, type, reference, metadata = {}, countAsEarned = Number(delta) > 0) {
  ...
  if (r.error) throw r.error;
  await transaction(guildId, userId, type, delta, reference, metadata);
  if (countAsEarned && Number(delta) > 0) await updateMission(guildId, userId, 'earn', Number(delta), false);
  return next;
}
...
async function handle(message) {
  ...
  try {
    ...
    if (cmd === '?customize' || cmd === '?paint') {
      ...
      const r = await sb.from('user_vehicles').update({ custom }).eq('id', o.id);
      if (r.error) throw r.error;
    ...
    if (cmd === '?upgrade') {
      ...
      const r = await sb.from('user_vehicles').update({ upgrades }).eq('id', o.id);
      if (r.error) throw r.error;
