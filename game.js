...
function dealershipEmbed(page=0){ const per=5,total=Math.ceil(V.length/per),p=Math.max(0,Math.min(page,total-1)); const slice=V.slice(p*per,p*per+per); const e=new EmbedBuilder().setTitle('🏪 VEHICLE LIFE DEALERSHIP').setDescription(slice.map(v=>`**#${v.id} ${v.name}** • ${v.rarity} • Class ${v.class}\n💰 ${money(v.price)} • ⚡ ${v.performance}`).join('\n\n')).setFooter({text:`Page ${p+1}/${total} • ?buy <vehicle>`}); return e; }
async function doRace(guild,member,arg,type='race',opponentMember=null){
  const p=await profile(guild.id,member.id); let owneds=await vehiclesOf(guild.id,member.id); if(!owneds.length) return '❌ You need a vehicle first. Use `?dealership` and `?buy <vehicle>`.'; let active=owneds.find(x=>x.vehicle_id===p.active_vehicle_id)||owneds[0]; const v=V[active.vehicle_id-1]; let bUser=null,bv=null,bOwner=null;
  if(opponentMember){ const bp=await profile(guild.id,opponentMember.id); const bo=await vehiclesOf(guild.id,opponentMember.id); bOwner=bo.find(x=>x.vehicle_id===bp.active_vehicle_id)||bo[0]; if(!bOwner) return `❌ ${opponentMember} has no vehicle.`; bv=V[bOwner.vehicle_id-1]; }
...
async function ensureLiveWorld(guildId){
  ...
  for(let i=open.length;i<templates.length;i++){
    ...
    const ins=await sb.from('events').insert({guild_id:gid,name:t.name,description:t.description,entry_fee:t.entry_fee,status:'open',starts_at:starts,ends_at:ends,rewards:t.rewards});
...
async function handle(message){
  ...
  try{
    ...
    if(cmd==='?race'||cmd==='?drag'||cmd==='?trackrace'||cmd==='?timetrial'){await syncLegacyOwnership(gid,uid); const target=message.mentions.members.first(); const type=cmd==='?drag'?'drag':cmd==='?trackrace'?'track':cmd==='?timetrial'?'time':'race'; return message.reply(await doRace(message.guild,message.member,args.join(' '),type,target));}
    ...
    return false;
  }catch(e){ console.error('Vehicle Life game error:',e); const msg=e.message==='INSUFFICIENT_FUNDS'?'❌ Insufficient funds.':`❌ Game system error: ${e.message}`; await message.reply(msg).catch(()=>{}); return true; }
}
